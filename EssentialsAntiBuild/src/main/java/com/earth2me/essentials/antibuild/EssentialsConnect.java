...
class EssentialsConnect {
    ...
    EssentialsConnect(final Plugin essPlugin, final IAntiBuild antibuildImpl) {
        ess = (IEssentials) essPlugin;
        protect = antibuildImpl;
    ...
    void alert(final User user, final String item, final String tlKey) {
        ...
        ess.getLogger().log(Level.WARNING, AdventureUtil.getAdventureFacet().miniToLegacy(tlLiteral("alertFormat", user.getName(), tlLiteral(tlKey), item, loc.getWorld().getName() + "," + loc.getBlockX() + "," + loc.getBlockY() + "," + loc.getBlockZ())));
