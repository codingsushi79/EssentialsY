...
import net.essentialsy.config.SplitConfigLoader;
...
public class Settings implements net.ess3.api.ISettings {
    ...
    private final transient SplitConfigLoader splitConfigLoader;
    ...
    public Settings(final IEssentials ess, final ModulesConfig modulesConfig) {
        this.ess = ess;
        config = new EssentialsConfiguration(new File(ess.getDataFolder(), "config.yml"));
        splitConfigLoader = new SplitConfigLoader((Essentials) ess, modulesConfig);
        reloadConfig();
    }
    ...
    public void reloadConfig() {
        splitConfigLoader.load();
        if (splitConfigLoader.usesSplitConfig()) {
            config.applyExternalRoot(splitConfigLoader.getMergedNode());
        } else {
            config.load();
        }
